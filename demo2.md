...
# Steps:
1. Create a New Apache Airlfow Job:
...
5. Confiture the dbt Project File (dbt_project.yml):
...
8. Define the Source Table:
   - To use {{ source('SalesDemoLakehouse', 'raw_sales') }} in your dbt models, you need to tell dbt about your source tables in the Lakehouse.
  - In the Airflow IDE, navigate to the models folder within your dbt project (dags/sales_dbt_transforms/models).
...
You have successfully:
  - Set up a Fabric Apache Airflow job (as a dbt environment).
  - Configure a dbt project to connect to your Fabric Lakehouse.
  - Define a data transformation using a dbt model.
  - Run the dbt transformation manually using the dbt CLI within the Airflow IDE.
  - Verify the transformed data in the Lakehouse.
